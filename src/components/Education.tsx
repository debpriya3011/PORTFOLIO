    ...
    >
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-base sm:text-lg mb-1 text-foreground">{item.institution}</h4>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs font-mono text-muted-foreground">
            ...
            {item.grade && (
              <span className="px-2 py-0.5 rounded-md bg-muted border border-border text-foreground/80 text-[11px] font-mono">
                Grade: {item.grade}
              </span>
            )}
          </div>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-base mb-1 text-foreground">{cert.name}</h4>
          <p className="text-muted-foreground font-medium text-xs sm:text-sm">{cert.issuer}</p>
          {issueDate && (
          ...
          {skillsArray.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2.5">
              {skillsArray.slice(0, 3).map((skill: string) => (
                <span
                  key={skill}
                  className="px-2 py-0.5 rounded-md bg-muted border border-border text-[11px] font-mono text-muted-foreground"
                >
                  {skill}
                </span>
              ))}
              {skillsArray.length > 3 && (
                <span className="px-2 py-0.5 rounded-md bg-muted border border-border text-[11px] font-mono text-muted-foreground">
                  +{skillsArray.length - 3}
                </span>
              )}
            </div>
          ...
          {credentialUrl && (
            <Button
              variant="outline"
              size="sm"
              className="mt-3 text-xs h-7 border-border hover:bg-accent"
